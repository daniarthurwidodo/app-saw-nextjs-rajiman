...
export class TasksService {
  ...
  static async createTask(
    ...
    try {
      ...
      // Create new task
      logger.debug(
        { title: sanitizedData.title, createdBy },
      ...
      const createdTask = await prisma.task.create({
        data: {
          ...
          dueDate: sanitizedData.due_date
            ? new Date(sanitizedData.due_date)
            : null,
        },
      });
  ...
  static async updateTask(
    ...
    try {
      ...
      if (sanitizedData.assigned_to) {
        logger.debug(
          { taskId, assignedTo: sanitizedData.assigned_to },
          "Validating assigned user",
        );
        const user = await prisma.user.findFirst({
          where: {
      ...
      if (sanitizedData.title !== undefined)
        updatePrismaData.title = sanitizedData.title;
      ...
      if (sanitizedData.status !== undefined)
        updatePrismaData.status = sanitizedData.status;
      if (sanitizedData.priority !== undefined)
        updatePrismaData.priority = sanitizedData.priority;
      if (sanitizedData.due_date !== undefined)
        updatePrismaData.dueDate = sanitizedData.due_date
          ? new Date(sanitizedData.due_date)
          : null;
