  ...
  try {
    ...
    async function rollbackUsers() {
      try {
        ...
        errors.push(
          `Failed to rollback users: ${error instanceof Error ? error.message : 'Unknown error'}`
        );
      }
    }
    ...
    async function rollbackSampleData() {
      try {
        ...
        errors.push(
          `Failed to rollback sample data: ${error instanceof Error ? error.message : 'Unknown error'}`
        );
      }
    }
